...
const partners = ["Zillow", "REALTOR", "Trulia", "realtor.com", "Homes.com", "REDFIN"];
  ...
  return (
    <main className="font-nunito text-brand-accent">
      ...
      <section className="m-auto flex w-full max-w-310 flex-col justify-center gap-10 px-5 py-16 lg:flex-row lg:justify-between">
        ...
        <div className="relative min-h-70 w-full max-w-xl overflow-hidden rounded-2xl bg-brand-navy lg:min-h-90">
          <Image src="/hero1.jpg" alt="EstateHub living room" fill className="object-cover" />
      ...
      <section className="overflow-hidden border-y border-slate-200 bg-slate-50 py-8">
        ...
        <div className="partner-marquee mt-7 overflow-hidden">
          <div className="partner-marquee-track flex w-max whitespace-nowrap text-xl font-bold text-brand-navy/60">
            {[0, 1].map((group) => (
              <div key={group} aria-hidden={group === 1} className="flex shrink-0 items-center gap-12 pr-12">
                {partners.map((brand) => <span key={`${group}-${brand}`}>{brand}</span>)}
              </div>
            ))}
          </div>
        </div>
